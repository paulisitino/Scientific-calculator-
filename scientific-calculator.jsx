...
function applyOp(a, op, b) {
  switch (op) {
    ...
    case "ʸ√x": return Math.pow(b, 1 / a);
...
export default function Calculator() {
  ...
  const reset = () => {
    setDisplay("0"); setExpr(""); setFirstOp(null);
    setPendingOp(null); setJustEvaled(false); setOpenParens(0);
  };
  ...
  const evaluate = useCallback(() => {
    if (pendingOp && firstOp !== null) {
      ...
      const result = applyOp(a, pendingOp, b);
  ...
  }, [pendingOp, firstOp, display, expr]);
  ...
  const handleButton = (val) => {
    ...
    if (val === "Rand") {
      const r = formatNum(Math.random());
      setDisplay(r); setExpr("Rand"); setJustEvaled(true); return;
    }

    // --- Digits & dot ---
    if (isDigit(val)) {
      if (justEvaled) { setDisplay(val); setExpr(""); setJustEvaled(false); return; }
    ...
    // --- Parentheses (simple open/close counting) ---
    if (val === "(") {
      setExpr(expr + "("); setOpenParens(o => o + 1);
      setDisplay("0"); setJustEvaled(false); return;
    }
    if (val === ")") {
      if (openParens === 0) return;
      setExpr(expr + display + ")"); setOpenParens(o => o - 1);
      setJustEvaled(true); return;
    }

    // --- Two-operand ops ---
    if (isOp(val) || val === "xʸ" || val === "ʸ√x" || val === "EE") {
      if (pendingOp && !justEvaled) {
        const r = applyOp(firstOp, pendingOp, cur);
        setDisplay(formatNum(r));
        setFirstOp(r);
        setExpr(formatNum(r) + " " + val + " ");
      } else {
        setFirstOp(cur);
        setExpr(display + " " + val + " ");
      }
  ...
  const btnStyle = (val) => {
    const base = {
      ...
      userSelect: "none", width: "52px", height: "52px",
    };
    if (val === "=")  return { ...base, background: "#ff9f0a", color: "#fff" };
    if (isOp(val))    return { ...base, background: "#ff9f0a", color: "#fff" };
    if (["AC","±","x%","mc","m+","m-","mr","2nd","Rad","Deg"].includes(val))
      return { ...base, background: "#636366", color: "#fff", fontSize: "12px" };
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: "min(380px, 96vw)", background: "#000",
        borderRadius: "20px", padding: "16px",
        boxShadow: "0 0 60px rgba(255,159,10,0.08)",
      }}>
        {/* Display */}
        <div style={{ textAlign: "right", padding: "12px 8px 4px", minHeight: "90px" }}>
          <div style={{ color: "#636366", fontSize: "13px", minHeight: "20px", marginBottom: "4px", wordBreak: "break-all" }}>
            {expr || "\u00a0"}
          </div>
          <div style={{
            color: "#fff",
            fontSize: display.length > 10 ? "32px" : "52px",
            fontWeight: "200", letterSpacing: "-1px",
            lineHeight: 1, wordBreak: "break-all",
            transition: "font-size 0.15s",
          }}>
            {display}
          </div>
          <div style={{ color: "#636366", fontSize: "11px", marginTop: "4px" }}>
        ...
        <div style={{ display: "grid", gap: "10px", marginTop: "12px" }}>
          {BUTTONS.map((row, ri) => (
            <div key={ri} style={{ display: "grid", gridTemplateColumns: "repeat(10, 1fr)", gap: "8px" }}>
              {row.map((val) => {
                // map 2nd-mode labels
                let label = val;
                if (is2nd) {
                  const map = { sin:"sin⁻¹", cos:"cos⁻¹", tan:"tan⁻¹", sinh:"sinh⁻¹", cosh:"cosh⁻¹", tanh:"tanh⁻¹", "x²":"x^y", "√x":"ʸ√x", "eˣ":"yˣ", "10ˣ":"2ˣ", ln:"logᵧ", log:"log₂" };
                  label = map[val] || val;
                }
                return (
                  <button
